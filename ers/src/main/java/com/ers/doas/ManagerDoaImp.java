...
public class ManagerDoaImp implements ManagerDoa{
	private static Logger logger = LogManager.getLogger(ManagerDoaImp.class);
	@Override
	public Manager selectManager(String username, String password) throws PasswordException, UsernameException {
		...
		try {
			...
			logger.error("\u001B[31m" + "ManagerDoa (selectManager): The manager email saved in the database for username: " + username + " is not a valid email" + "\u001B[0m");
		} catch(PasswordException e) {
			logger.error("\u001B[31m" + "ManagerDoa (selectManager): The manager password saved in the database for username: " + username + " is not a valid password" + "\u001B[0m");
		}
		return manager;
	}
}
